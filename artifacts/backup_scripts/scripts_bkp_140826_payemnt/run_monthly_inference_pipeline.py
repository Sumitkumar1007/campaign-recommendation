...
def compute_model_drift_metrics(
    *,
    ...
    if not train_feature_file.exists():
        fallback_path = FEATURE_DATA_DIR / "strategy_monthly_features_inference.csv"
        if fallback_path.exists():
            print(f"[WARNING] Training feature file not found at {train_feature_file}. Falling back to: {fallback_path}")
            train_feature_file = fallback_path
        else:
            raise FileNotFoundError(f"Training feature file not found for drift calculation: {train_feature_file} and fallback {fallback_path} is also missing.")
    ...
    # --- 2.5 LOAD AND MERGE PAYMENT FEATURES FOR DRIFT CALCULATION ---
    target_tokens = []
    for m in (baseline_source_months or []) + [source_month_label]:
        if m:
            try:
                target_tokens.append(month_file_token(parse_month(str(m))))
            except Exception:
                pass
    ...
    for token in target_tokens:
        payments_path = PAYMENT_DATA_DIR / "payments"
        if payments_path.exists():
            for file in payments_path.glob(f"*{token}*.csv"):
                try:
                    df_p = pd.read_csv(file, usecols=["apac_card_number", "payment_datetime"]).copy()
                    df_p["APICARD"] = df_p["apac_card_number"].astype(str).str.strip()
                    df_p["MONTH"] = token
                    df_feat = df_p[["APICARD", "MONTH"]].copy()
                    df_feat["feature"] = "PAYMENT_RAW"
                    df_feat["count"] = 1
                    payment_dfs.append(df_feat)
                except Exception as e:
                    print(f"[PAYMENT LOG] Error reading payment file {file.name}: {e}")
                    
    payment_pivot = None
    if payment_dfs:
        payment_features = pd.concat(payment_dfs, ignore_index=True)
        payment_pivot = payment_features.pivot_table(
            index=["APICARD", "MONTH"],
            columns="feature",
            values="count",
            aggfunc="sum",
    ...
    if payment_pivot is not None:
        # Merge payments into inference rows
        if "MONTH" in prediction_rows.columns:
            prediction_rows["_MERGE_TOKEN"] = prediction_rows["MONTH"].apply(lambda m: month_file_token(parse_month(str(m))) if pd.notna(m) else "")
            prediction_rows = prediction_rows.merge(
                payment_pivot,
                left_on=["APAC_CARD_NUMBER", "_MERGE_TOKEN"],
                right_on=["APICARD", "MONTH"],
                how="left"
            )
            if "APICARD" in prediction_rows.columns:
                prediction_rows.drop(columns=["APICARD", "MONTH_y", "_MERGE_TOKEN"], inplace=True)
                prediction_rows.rename(columns={"MONTH_x": "MONTH"}, inplace=True)
                
        # Merge payments into baseline rows
        if "SOURCE_MONTH" in train_df.columns:
            train_df["_MERGE_TOKEN"] = train_df["SOURCE_MONTH"].apply(lambda m: month_file_token(parse_month(str(m))) if pd.notna(m) else "")
            ...
            if "APICARD" in train_df.columns:
                train_df.drop(columns=["APICARD", "MONTH", "_MERGE_TOKEN"], inplace=True)
...
def main() -> None:
    ...
    try:
        ...
        with log_step(logger, "fetch_payment_history", source_month=args.source_month):
            run_python_script(
                "fetch_payment_from_postgres.py",
                "--host", args.host,
                "--port", str(args.port),
                "--dbname", args.dbname,
                "--user", args.user,
                ...
                "--fetch-month", args.source_month,
                "--history-months", str(args.history_window_months + 1),
                logger=logger,
            )
