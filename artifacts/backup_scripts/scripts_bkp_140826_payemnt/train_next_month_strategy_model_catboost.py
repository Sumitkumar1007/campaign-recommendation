...
def prepare_dataset(
    ...
    # Extract unique months present in dataset
    months = dataset["SOURCE_MONTH"].dropna().unique().tolist() if "SOURCE_MONTH" in dataset.columns else []
    
    def parse_month(yyyy_mm: str) -> pd.Period:
        try:
            return pd.Period(yyyy_mm.strip(), freq="M")
        except Exception:
            raise ValueError(f"Invalid month format: {yyyy_mm}. Expected YYYY-MM.")
    ...
    if payment_pivot is not None:
        dataset["_MERGE_TOKEN"] = dataset["SOURCE_MONTH"].apply(lambda m: month_file_token(parse_month(str(m))) if pd.notna(m) else "")
        ...
        if "APICARD" in dataset.columns:
            dataset.drop(columns=["APICARD", "MONTH", "_MERGE_TOKEN"], inplace=True)
