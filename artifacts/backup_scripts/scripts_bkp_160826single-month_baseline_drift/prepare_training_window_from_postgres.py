...
def fetch_month_extracts(args: argparse.Namespace, logger: logging.Logger, months: list[str]) -> list[Path]:
    ...
    files: list[Path] = []
    query = build_query(args.schema, args.table, args.vertical_column)
    logger.info(
        "Opening source DB connection for training extract | host=%s port=%s dbname=%s schema=%s table=%s vertical_column=%s months=%s",
        args.host,
        args.port,
        args.dbname,
        args.schema,
...
def main() -> None:
    ...
    try:
        ...
        logger.info(
            "Preparing training window | months=%s host=%s dbname=%s schema=%s table=%s vertical_column=%s month_source=%s",
            args.months,
            args.host,
            args.dbname,
            args.schema,
